                                ...
                                <c:otherwise>
                                    <c:forEach var="listaprodutos" items="${produtos}">
                                        <tr>
                                            ...
                                            <td><img src="data:image/jpeg;base64,${listaprodutos.imagemBase64}"
                                                    alt="Imagem do Produto"></td> --%>
                                            <td><a href="alterarproduto.jsp" class="button">Alterar</a></td>
                                            <td><a href="imagem?id=${listaprodutos.ID}" class="button">Visualizar</a>
                                            </td>
                                            <td>
                                            <td>
                                                <form action="/alteraStatusPorduto" method="post"
                                                    id="formAlterarStatus${listaprodutos.ID}">
                                                    <input type="hidden" name="id" value="${listaprodutos.ID}">
                                                    <input type="hidden" name="novaStatus"
                                                        value="${listaprodutos.status}">
                                                    <button type="submit"
                                                        onclick="confirmarAlteracao(${listaprodutos.ID})">Alterar
                                                        Status</button>
                                                </form>
                                            </td>
                                            </td>
                                            ...
                                            <td><a
                                                    href="alterarproduto.jsp?id=${listaprodutos.ID}&nome=${listaprodutos.nome}&Quantidade=${listaprodutos.estoque}&AvaliacaoProduto=${listaprodutos.avaliacao}&DescricaoProduto=${listaprodutos.descricao}&preco=${listaprodutos.preco}">Alterar</a>
                                            </td>
